...
<template>
  <main v-fade-in>
    <section class="py-12 w-full max-w-screen-xl mx-auto">
      ...
      <p data-fade class="mt-2 text-#2f3f5b dark:text-gray-300 transition-colors duration-500">Articles about Some of My Whimsical Ideas.</p>
      ...
      <div data-fade class="mt-4 flex flex-wrap items-baseline justify-start gap-2 text-sm text-#2f3f5b dark:text-gray-400 transition-colors duration-500">
        ...
        <button v-for="tag in allTags" :key="tag" @click="toggleTag(tag)" 
                ...
                class="inline-block px-2 py-1 text-xs rounded-full transition-colors duration-500 cursor-pointer shadow-none border-none">
          {{ tag }}
        </button>
      </div>
      <ul class="mt-8 grid gap-4 grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3">
        <li v-for="post in filteredPosts" :key="post.slug + selectedTag + searchTerm" data-fade @animationend="() => markRevealed(post.slug)" :class="'relative w-full h-160px rounded-xl list-none will-change-transform motion-safe:transform-gpu transition duration-500 animate-shadow backdrop-blur-2 bg-black/3 dark:bg-white/10 ' + (revealedPosts.has(post.slug) ? 'hover:shadow-[0_0_0_1px_#00e699] hover:scale-[1.02] active:scale-[0.97] cursor-pointer' : '')">
          <div class="block h-full rounded-md p-4 no-underline focus:outline-none focus-visible:ring focus-visible:ring-[#00e699] transition-transform duration-500 active:scale-95 hover:scale-102" @click="delayedNavigate(`/posts/${post.slug}.html`)">
            <h4 class="text-#2f3f5b dark:text-white text-xl transition-colors duration-500">{{ post.title }}</h4>
            <p class="mt-1 text-#2f3f5b dark:text-gray-400 text-sm transition-colors duration-500">{{ post.description }}</p>
            ...
            <div v-if="post.tags.length" class="absolute right-4 bottom-4 flex flex-wrap gap-1">
              <span v-for="tag in post.tags" :key="tag" class="px-2 py-1 text-xs rounded-full bg-black/5 text-#2f3f5b dark:bg-white/10 dark:text-white/80 transition-colors duration-500">{{ tag }}</span>
            </div>
          </div>
        </li>
      </ul>
